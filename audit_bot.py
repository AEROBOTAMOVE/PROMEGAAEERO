...
def check_honesty(code_dir: Path, data_dir: Path):
    ...
    # 🔴 ОДИТ-64 · Ч2 ТВЪРДЕШЕ «БИМОДАЛНО» С ЧИСЛА ОТ ДРУГА ГЕОМЕТРИЯ.
    # Четеше `tp_hits`, а _meta на самата статистика казва: «tp_hits.* остават
    # от СТАРИЯ файл — НЕ са проверени под доставената геометрия».
    # F23 (12.08) го измери наново върху 6846 сдвоени входа:
    #     стигат ТП3       58.9% (старо)  →  25.5% (измерено)
    #     живеят по средата 11.9% (старо)  →  43.2% (измерено)
    # Тоест разпределението НЕ е бимодално и стълбата РАБОТИ. F23 го потвърди
    # и отдругаде: «всичко на ТП3» е ПО-ЛОШО (−0.172$ срещу −0.094$).
    _тд = st.get("tp_hits_доставена") or {}
    if _тд.get("живеят_между") is not None:
        _мж = float(_тд["живеят_между"])
        if _мж < 20:
            A.warn(cat, "Ч2", "разпределението е БИМОДАЛНО",
                   f"само {_мж:.1f}% живеят между ТП1 и ТП3 (мерено под доставената)",
                   "стълбицата от 3 тейка е предимно театър — знай го")
        else:
            A.ok(cat, "Ч2", "стълбицата работи",
                 f"{_мж:.1f}% живеят между ТП1 и ТП3 · ТП3 {_тд.get('стигат_тп3')}% "
                 f"(F23, доставената геометрия, n={_тд.get('n')})")
    else:
        A.skip(cat, "Ч2", "стълбицата",
               "няма tp_hits_доставена — старите tp_hits са от ДРУГА геометрия "
               "и не се ползват")
