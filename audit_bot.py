...
def telegram_msg(R, Y):
    ...
    for _x in A.yellows:
        if НЕ_МОЖАХ.search(f"{_x.get('name','')} {_x.get('detail','')}"):
            _x["level"] = SKP
    A.yellows = [x for x in A.yellows if x["level"] == YEL]
    ...
    if _nr:
        L.append(f"🔴 <b>{_nr} {'ПРОБЛЕМ' if _nr == 1 else 'ПРОБЛЕМА'} — виж долу</b>")
        L.append(f"<i>останалите {_grn} проверки са наред.</i>")
    elif _new:
        # ОДИТ-18: няма счупено → ЗЕЛЕНО заглавие. Жълтото е бележка, не тревога.
        L.append("✅ <b>ВСИЧКО РАБОТИ</b>")
        L.append(f"<i>{_grn} проверки минаха. "
                 f"{len(_new)} {'бележка' if len(_new) == 1 else 'бележки'} по-долу — "
                 f"нищо счупено.</i>")
    else:
        L.append("✅ <b>ВСИЧКО РАБОТИ</b>")
        L.append(f"<i>{_grn} проверки минаха. Нито едно предупреждение.</i>")
    L.append("─────────────────")
