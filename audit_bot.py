...
ПОЗДРАВИ = [
    "Здрасти. Минах през всичко.",
    "Проверих бота. Ето накратко.",
    "Готово — обиколих всяка проверка.",
    "Тук съм. Ето какво заварих.",
    "Прегледах го от край до край.",
    "Обиколката е готова.",
]


def telegram_msg(R, Y):
    # ОДИТ-21: заглавието да е човешко, не системно. Поздравът се сменя по деня,
    # за да не е един и същ ред три пъти дневно.
    _h = sofia(now_utc())
    _чс = "Добро утро" if _h.hour < 12 else ("Добър ден" if _h.hour < 18 else "Добър вечер")
    _пз = ПОЗДРАВИ[_h.timetuple().tm_yday % len(ПОЗДРАВИ)]
    L = [f"🤖 <b>{_чс}!</b> · {_h:%d.%m, %H:%M}",
         f"<i>{_пз}</i>",
         "─────────────────"]
    # ОДИТ-18: първо свали от ⚠️ всичко, което значи само «не можах да проверя».
    # 🔴 ОДИТ-21 — БЪГ, КОЙТО КАЧИХ И КОЙТО ЧУПЕШЕ ОДИТА В CI:
    # тук имаше `A.yellows = [...]`, а `yellows` е @property БЕЗ setter →
    # AttributeError при всяко пускане с --send. Локално не се виждаше, защото
    # `telegram_msg` се вика САМО при --send. Одитът падна два пъти, преди да го хвана.
    # Присвояването беше и ИЗЛИШНО: `yellows` се извежда от `rows` по ниво, значи
    # смяната на `_x["level"]` по-долу сама го маха от списъка.
    ...
    _grn = sum(1 for x in A.rows if x["level"] == GRN)
    if _nr:
        L.append(f"<b>Има {_nr} {'нещо за оправяне' if _nr == 1 else 'неща за оправяне'}</b>")
        L.append(f"<i>Другите {_grn} проверки са наред — само тези долу искат внимание.</i>")
    elif _new:
        # ОДИТ-18: няма счупено → ЗЕЛЕНО заглавие. Жълтото е бележка, не тревога.
        L.append("✅ <b>Всичко работи.</b>")
        L.append(f"<i>Минаха {_grn} проверки. Има {len(_new)} "
                 f"{'бележка' if len(_new) == 1 else 'бележки'} по-долу — нищо счупено, "
                 f"просто да ги знаеш.</i>")
    else:
        L.append("✅ <b>Всичко работи.</b>")
        L.append(f"<i>Минаха {_grn} проверки. Нито една бележка — рядко се случва.</i>")
    L.append("─────────────────")
    # ЧЕРВЕНОТО първо и с лекарството; жълтото след него, без лекарство
    # ОДИТ-22: БЕЗ ❌ и ⚠️ в съобщението. Собственикът изрично поиска да ги няма.
    # Състоянието се казва с ДУМИ: счупеното е «→», бележката е «·».
    # Знаците остават в конзолата (за мен), не в Телеграм (за него).
    for x in sorted(_new, key=lambda z: 0 if z["level"] == RED else 1)[:10]:
        ic = "→" if x["level"] == RED else "·"
    ...
    if _sk:
        L.append(f"⊘ <i>не можах да проверя ({len(_sk)}): "
                 f"{_esc(', '.join(x['code'] for x in _sk))} — нямаше какво да меря "
                 f"(без отворена сделка / без барове / източник мълчи). Не е повреда.</i>")
    if _kn:
        L.append(f"📌 <i>знам за тях и са приети: {_esc(', '.join(x['code'] for x in _kn))}</i>")
    L.append("─────────────────")
    L.append("<i>Следващата обиколка е след няколко часа. Ако нещо се счупи междувременно, ще го видиш тук.</i>")
    return "\n".join(L)
