...
for _z26 in (None, ("A", "зона A"), ("B", "зона B"), ("C", "зона C")):
    for _bal26 in (500, 1000, 5000, 25000):
        ...
        _mm = _re26.search(r"(\d+\.\d\d) лот · риск \$(\d+)", _re26.sub(r"<[^>]+>", "", _m26))
        if _mm:
            _лот26, _риск26 = float(_mm.group(1)), float(_mm.group(2))
            ck(f"П26 риск={_риск26:.0f}$ отговаря на {_лот26:.2f} лот (баланс {_bal26}, зона {(_z26 or (chr(45),))[0]})",
               abs(_лот26 * 100.0 * lb.SL_D - _риск26) < 1.0)

# ═══ П27 · ПОЗДРАВЪТ НА ОДИТ-РОБОТА (ОДИТ-29) ══════════════════════════════
# Коста: «махни това добър ден — направи го някакъв поздрав Коста тука съм».
