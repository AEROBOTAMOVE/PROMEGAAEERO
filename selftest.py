...
try:
    ...
    ck("П20 шорт: при ИЗКЛЮЧЕН флаг геометрията е замразената (7.5/12/20/−20)",
...
ck("П20 шорт: при ВКЛЮЧЕН флаг е неговата (5 / 10 / 20 / −15)",
   abs((_E20 - _LV20["tp1"]) - 5.0) < 1e-6 and abs((_E20 - _LV20["tp2"]) - 10.0) < 1e-6
   and abs((_E20 - _LV20["tp3"]) - 20.0) < 1e-6 and abs((_LV20["sl"] - _E20) - 15.0) < 1e-6)
...
ck("П103 стопът е −20.00$ = 200 пипса", abs((4650.0 - _л103["sl"]) - 20.0) < 0.011)
...
try:
    ...
    ck("П103 при ИЗКЛЮЧЕН флаг шортът е огледален (7.5 / 20)",
...
ck("П103 стопът на шорта е ПО-КЪС от този на лонга (това е ключът)",
   (_с103["sl"] - 4650.0) < (4650.0 - _л103["sl"]))
...
ck("П50 и стопът на ШОРТА е ПО-КЪС — това е ключът",
   abs(_н50с["sl"] - 4400) < abs(_н50["sl"] - 4400))
...
for _к134, _оч134 in (("tp1", 75.0), ("tp2", 120.0), ("tp3", 200.0), ("sl", 200.0)):
...
for _к134, _оч134 in (("tp1", 50.0), ("tp2", 100.0), ("tp3", 200.0), ("sl", 150.0)):
...
try:
    ...
    ck("П134 ШОРТ_ГЕОМЕТРИЯ=не връща 75/120/200 ДОСЛОВНО",
