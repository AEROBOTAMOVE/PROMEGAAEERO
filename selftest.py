...
ck("П23 анти-спамът вече също си казва причината с ЧИСЛА",
   "правилото пуска до {REOFFER_MAX_AGE_H}ч" in _src
   and "от последната карта, трябват" in _src)
...
# 🔴 ЖЕЛЯЗНО: НЕ се отварят двете врати заедно. Реплеят на целия журнал
# (4342 ръна) брои REOFFER_H+MAX_AGE заедно = +399 карти = 21/ден = карта на
# всеки 68 минути. Спамът не е сигнал.
ck("П89 таванът на възрастта НЕ е пипан (двете врати не се отварят заедно)",
   lb.REOFFER_MAX_AGE_H == 12.0)
