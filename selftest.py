...
ck("П17 клас B → препоръчва по-малко от пълния", "ВЛЕЗ" in _b17 and "пълен размер" not in _b17)
ck("П17 клас C → препоръчва по-малко от пълния", "ВЛЕЗ" in _c17 and "пълен размер" not in _c17)
ck("П17 A/B/C дават ТРИ РАЗЛИЧНИ препоръки за размер",
   len({[x for x in т.split(chr(10)) if "ВЛЕЗ" in x][0]
        for т in (_a, _b17, _c17)}) == 3)
ck("П17 клас A получава ПЪЛЕН размер и го КАЗВА",
   "ВЛЕЗ СИЛНО" in _a and "пълен размер" in _a)
...
_рз39 = lambda т: [x for x in т.split(chr(10)) if "ВЛЕЗ" in x]
...
ck("П39 думата и дялът не си противоречат (по-голям множител → не по-малка дума)",
   [d for _, d, _ in _шест39] == sorted([d for _, d, _ in _шест39],
                                        key=lambda x: ["МНОГО ЛЕКО", "ЛЕКО",
                                                       "НОРМАЛНО", "СИЛНО"].index(x),
                                        reverse=True))
...
ck("П70 тишината е ИЗМЕРЕНА и записана във файла",
   isinstance(_т70.get("тиха_пауза_медиана_дни"), int)
   and isinstance(_т70.get("тиха_пауза_максимум_дни"), int))
...
ck("П70 картата казва КОЛКО обикновено трае паузата",
   "траят обикновено" in _с70 and str(_т70["тиха_пауза_медиана_дни"]) in _с70)
ck("П70 и казва най-дългата мерена", str(_т70["тиха_пауза_максимум_дни"]) in _с70)
...
ck("П70 числата НЕ са заковани в кода",
   'тиха_пауза_медиана_дни' in _ист70
   and _ист70.count("най-дългата мерена: 30") == 0)
