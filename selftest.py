...
ck("О4 таванът е МАХНАТ (0 = без таван)", lb.REOFFER_MAX_AGE_H == 0)
ck("О4 честотата се пази от ПРАГА, не от тавана",
...
ck("О4 таванът по клетка е БЕЗКРАЕН и в двете клетки",
   lb._max_age_h("long", 2) == float("inf")
   and lb._max_age_h("long", 5) == float("inf"))
ck("О4 най-старият жив ключ (115ч) вече МИНАВА през тавана",
   lb._max_age_h("long", 2) > 115 and lb._max_age_h("long", 5) > 115)
...
ck("О5 стар сетъп (27.4ч) НЕ показва часовете", "27ч" not in _stm
   and "нивата са от сега" in _stm and "без таван" in _stm)
...
ck("П90 в отпушен режим НИТО ЕДНА клетка не спира по възраст",
   all(lb._max_age_h(_д, _н) == float("inf")
       for _д in ("long", "short", None, "")
       for _н in (0, 1, 2, 3, 5, 9)))
ck("П90 суровите стойности са 0 (а не изтрити)",
   lb.REOFFER_MAX_AGE_H == 0 and lb.MAX_AGE_ПРЕСЕН == 0)
ck("П90 живият сетъп на 258ч вече МИНАВА", 258.0 <= lb._max_age_h("long", 5))
...
ck("П90 таванът е МАХНАТ, не вдигнат", lb.REOFFER_MAX_AGE_H == 0.0)
