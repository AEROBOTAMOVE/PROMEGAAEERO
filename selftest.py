...
ck("П127 подразбирането е ВКЛЮЧЕНО и за двата (решение на собственика)",
   lb.ЛОНГ_САМО_НАД_SMA200 is True and lb.БЕЗ_ЛОНГ_ПЕТЪК is True)
ck("П127 включени → лонг под SMA200 в петък се ОТКАЗВА",
   lb._лонг_флагове("long", _под127, _пт127)[0] is False)
...
ck("П127 пътят назад съществува в кода (env «не» изключва)",
   'os.environ.get("ЛОНГ_САМО_НАД_SMA200", "да") == "да"' in _ж127пн
   and 'os.environ.get("БЕЗ_ЛОНГ_ПЕТЪК", "да") == "да"' in _ж127пн)
...
ck("П106 «НУЛА» не е изтрита от кода — пази се за истинска нула",
   '"НУЛА"' in _ИЗХОД and 'стълба > 0.005' in _ИЗХОД)
