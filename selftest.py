...
# ── вътре в прозореца БРОИ, извън него НЕ ──
for _ч107, _оч107 in ((0.0, 1), (1.0, 1), (3.9, 1), (4.0, 0), (9.0, 0), (25.0, 0)):
    ck("П107 стоп преди %.1fч → пазачът брои %d" % (_ч107, _оч107),
       lb._пазач_n(_g107(_ч107), "long", _N107) == _оч107)

# 🔴 ОБРАТНАТА ПОСОКА №1 · БЕЗ клеймо (стар guard.json) пази СТАРОТО
# поведение. Иначе едно качване щеше да отпуши мълчаливо всичко наведнъж.
ck("П107 без клеймо пази старото поведение", lb._пазач_n({"long": 1}, "long", _N107) == 1)
ck("П107 счупено клеймо пази старото поведение",
   lb._пазач_n({"long": 1, "long_t": "боклук"}, "long", _N107) == 1)
ck("П107 нула стопа си остава нула", lb._пазач_n(_g107(0.0, 0), "long", _N107) == 0)

# 🔴 ОБРАТНАТА ПОСОКА №2 · ПЪТЯТ НАЗАД РАБОТИ. Проверено с ИЗПЪЛНЕНИЕ на
# втори внос със сменена среда, не с четене на коментар.
import importlib.util as _iu107
import os as _os107

_стар107 = _os107.environ.get("ПАЗАЧ_ПРОЗОРЕЦ_Ч")
_os107.environ["ПАЗАЧ_ПРОЗОРЕЦ_Ч"] = "0"
try:
    ...
    try:
        ...
        pass
    ck("П107 ПАЗАЧ_ПРОЗОРЕЦ_Ч=0 връща «до края на деня»",
       _lb107.ПАЗАЧ_ПРОЗОРЕЦ_Ч == 0
       and _lb107._пазач_n(_g107(20.0), "long", _N107) == 1)
finally:
    if _стар107 is None:
        _os107.environ.pop("ПАЗАЧ_ПРОЗОРЕЦ_Ч", None)
    else:
        _os107.environ["ПАЗАЧ_ПРОЗОРЕЦ_Ч"] = _стар107
