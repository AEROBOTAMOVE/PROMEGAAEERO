...
ck("О4 глобалният REOFFER_H си остава 6", lb.REOFFER_H == 6)
...
ck("О4 и таванът по клетка НЕ може да е безкраен (стар сетъп пак умира)",
   lb.MAX_AGE_ПРЕСЕН < 999 and lb._max_age_h("long", 2) <= 48)
...
ck(f"П23 спирачките са 11 (7 оригинални + 4 от анти-спама)",
   _src.count("_спрян = (") == 11)
...
ck("П23 анти-спамът си казва причината с ЧИСЛА",
   "правилото пуска до {_тав:.0f}ч" in _src
   and "от последната карта, трябват {_прг:.0f}ч" in _src)
ck("П23 и казва В КОЯ КЛЕТКА важи това число (иначе числото е необяснимо)",
   _src.count("(клетка {_клт})") >= 2)
...
for _д23, _с23 in (("long", 2), ("long", 5), ("long", 1), ("short", 2)):
    ck(f"П23 текстът и решението ползват едно число ({_д23}/{_с23})",
       lb._max_age_h(_д23, _с23) == (lb.MAX_AGE_ПРЕСЕН
...
_нар72 = []
for _b in (4379.9, 4379.5, 4378.0, 4376.0, 4370.0, 4360.0):
    _р = _т72(_b, _b + 0.5)
...
_нарs72 = []
for _a in (4420.1, 4420.5, 4422.0, 4424.0, 4430.0):
...
ck(f"П74 после — точно на 2 часа ({sorted(set(_раз5))})", set(_раз5) == {2.0})
ck(f"П74 и при 15-минутна крачка пак на 2 часа",
   set(round(_к15[i] - _к15[i + 1], 2) for i in range(len(_к15) - 1)) == {2.0})
ck(f"П74 картите са повече от една ({len(_к5)})", len(_к5) >= 6)
...
ck("П74 картата обещава следващото напомняне", "ще ти напомня пак" in _т74)
...
ck("П86 под 200-дневната размерът се свива (мерено −1.25$)",
   lb._режим_тегло("long", {"below_sma200": True, "low_vol": False})[0] < 1.0)
ck("П86 и картата КАЗВА защо",
...
ck("П86 има път назад през средата",
   'os.environ.get("РЕЖИМ_W_ПОД"' in _ИЗХОД)
...
ck("П87 COOL_FLIP_MIN също", isinstance(lb.COOL_FLIP_MIN, int) and lb.COOL_FLIP_MIN == 15)
...
ck("П87 и стоп-пазача", "guard.get(new_dir, 0) < 2" in _ИЗХОД)
...
ck("П90 таванът е ПО КЛЕТКА, не глобален (основата остава 12ч)",
   lb.REOFFER_MAX_AGE_H == 12.0 and lb.MAX_AGE_ПРЕСЕН > lb.REOFFER_MAX_AGE_H)
ck("П90 ЛОНГ · fresh получава широкия таван (мерено +3.74$ с чист интервал)",
   lb._max_age_h("long", 2) == lb.MAX_AGE_ПРЕСЕН
   and lb._max_age_h("long", 3) == lb.MAX_AGE_ПРЕСЕН)
# 🔴 ОБРАТНАТА ПОСОКА — всяка клетка, чието мерене НЕ е чисто, остава на 12ч
ck("П90 ЛОНГ · day1 ОСТАВА на 12ч (мереното е разбъркано: +0.09/−1.65/+0.45)",
   lb._max_age_h("long", 1) == lb.REOFFER_MAX_AGE_H)
ck("П90 ЛОНГ · stale ОСТАВА (мерено −1.599$)",
   lb._max_age_h("long", 5) == lb.REOFFER_MAX_AGE_H)
ck("П90 ЛОНГ · mixed ОСТАВА", lb._max_age_h("long", 0) == lb.REOFFER_MAX_AGE_H)
ck("П90 ШОРТЪТ ОСТАВА непокътнат във ВСЯКА клетка (мерено −0.279$)",
   all(lb._max_age_h("short", n) == lb.REOFFER_MAX_AGE_H for n in (0, 1, 2, 3, 5, 9)))
ck("П90 липсваща посока не гърми и не отваря нищо",
   lb._max_age_h(None, 2) == lb.REOFFER_MAX_AGE_H
   and lb._max_age_h("", 3) == lb.REOFFER_MAX_AGE_H)
# синхронът с _cell_name се проверява чрез ИЗПЪЛНЕНИЕ на всеки стрийк
for _с90 in range(0, 10):
    _о90 = (lb.MAX_AGE_ПРЕСЕН if lb._cell_name(_с90) == "fresh" else lb.REOFFER_MAX_AGE_H)
    ck(f"П90 стрийк {_с90} ({lb._cell_name(_с90)}) → таван {_о90:.0f}ч",
       lb._max_age_h("long", _с90) == _о90)
...
ck("П90 нито прагът, нито таванът са пипани ГЛОБАЛНО",
   lb.REOFFER_H == 6.0 and lb.REOFFER_MAX_AGE_H == 12.0)
...
ck("П98 таванът е 5 по подразбиране", lb.ТАВАН_СДЕЛКИ == 5)
...
ck("П98 пазачът брои еднакво и за двата реда",
   _ИЗХОД.count('== "sl" and not _вц2') == 1 and "_взета_цел = any(cum_hit.get(k)" in _ИЗХОД)
...
ck("П91 и двете поискани стълби са там",
   any("150/150" in x for x in _с91) and any("100/100" in x for x in _с91))
...
ck("П91 бележката казва, че пълният стоп удря ПОВЕЧЕ (иначе е безсмислена)",
   all(x[6] > x[5] for x in lb.СТЪЛБИ_ДОП)
   and "с пълен стоп" in _с91[0])
# 🔴 ЧИСЛАТА В РЕДА трябва да са ТЕЗИ от таблицата — изпълнено, не грепнато
for _им91, _a, _b, _c, _d, _у91, _уп91 in lb.СТЪЛБИ_ДОП:
    ck(f"П91 {_им91} · и двете числа са в реда ({_у91:.0f}%→{_уп91:.0f}%)",
       f"{_у91:.0f}%→{_уп91:.0f}%" in _с91[0])
