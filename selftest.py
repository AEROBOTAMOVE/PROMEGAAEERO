...
if _CB22 is not None:
    ...
    ck("П40 ГОТОВ е ВХОД — казва КУПИ и дава лот",
    ...
    ck("П40 входът дава вход И стоп с думи", "вход" in _вх40 and "стоп" in _вх40)
    ck("П40 целите на входа са с номера", "1️⃣" in _вх40 and "2️⃣" in _вх40)
    ck("П40 входът НЕ се преправя за мерено правило — казва си, че е нов",
       "нов метод · още без бектест" in _вх40)
    ck("П40 наблюдението и входът НЕ звучат еднакво",
...
ck("П40 мозъчният риск е под мереното правило (няма бектест)",
   'os.environ.get("МОЗЪК_РИСК_W", "0.4")' in _s40)
ck("П40 има път назад (МОЗЪК_РАНГ_ВХОД=99 = само наблюдения)",
   'os.environ.get("МОЗЪК_РАНГ_ВХОД", "3")' in _s40)
