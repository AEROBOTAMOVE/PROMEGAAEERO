...
def _стълби_редове(entry, direction, dec=2):
    ...
    for име, t1, t2, t3, sl, удря, упълн in СТЪЛБИ_ДОП:
        ...
        _ч.append(f"{име} <code>{_fmt(lv['tp1'], dec)}</code>|"
                  f"<code>{_fmt(lv['tp2'], dec)}</code>|"
                  f"<code>{_fmt(lv['tp3'], dec)}</code> "
                  f"🛑<code>{_fmt(lv['sl'], dec)}</code> {удря:.0f}%"
                  f"→{упълн:.0f}% с пълен стоп")
    return ["🪜 " + " · ".join(_ч)]
