...
def _sig_msg(direction, score, agree_n, tier_name, spot, bar_price, bar_ts, lv, entry,
    ...
    if лот < 0.01:
        ...
        L.append(f"💰 под мин. лот · 0.01 лот = риск ≈${_мин_р:.0f} = "
                 f"{_мин_п:.1f}% от парите ти"
                 + (f" ⚠️ над целта {risk_pct:g}%" if _мин_п > risk_pct + 0.05 else ""))
    else:
        ...
        _защо_нам = []
        if _zw < 0.999:
            _защо_нам.append("зоната")
        if _малък:
            _защо_нам.append("сигналът не е пресен")
        L.append(f"💰 {лот_окр:.2f} лот · риск ≈${риск:.0f}"
                 + (f" (намален · {' и '.join(_защо_нам)})" if _защо_нам else "")
                 + " · по 1/3 на цел")
    L.append(f"📌 {защо}")
            ...
            for _s in _setups:
                ...
                try:
                    ...
                    if _рм > 0.01 and int(_s.get("ранг", 0)) >= МОЗЪК_РАНГ_ВХОД:
                        ...
                        _лот_ред = ((f"💰 {_лот:.2f} лот · риск ≈${_лот * 100.0 * _рм:.0f}"
                                     f" · по 1/3 на цел") if _лот >= 0.01 else
                                    f"💰 под мин. лот · 0.01 лот = риск ≈${_рм:.0f}")
                except Exception:
                    _лот_ред = None
