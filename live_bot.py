...
ЛОНГ_САМО_НАД_SMA200 = os.environ.get("ЛОНГ_САМО_НАД_SMA200", "не") == "да"
...
БЕЗ_ЛОНГ_ПЕТЪК = os.environ.get("БЕЗ_ЛОНГ_ПЕТЪК", "не") == "да"
...
ШОРТ_ГЕОМЕТРИЯ = os.environ.get("ШОРТ_ГЕОМЕТРИЯ", "да") == "да"
...
ЛОНГ_СТОП_ПИПСА = float(os.environ.get("ЛОНГ_СТОП_ПИПСА", "130"))
...
ГЕЙТ_ЛОНГ_НЕДОКАЗАНО = os.environ.get("ГЕЙТ_ЛОНГ_НЕДОКАЗАНО", "да") == "да"
...
МАЛЪК_РАЗМЕР_W = float(os.environ.get("МАЛЪК_РАЗМЕР_W", "1.0"))
...
РЕЖИМ_W_ПОД = float(os.environ.get("РЕЖИМ_W_ПОД", "1.0"))    # под SMA200 (беше 0.5)
...
def _да_с_числа(seg, текст, fast=None):
    ...
    if СТАТ_КЛЕТКА_СТРОГО:
        return ("НЕ — тази клетка няма измерени числа (%d сделки, трябват %d)"
                % (_n, MIN_N)), False
    return (текст + " · ⚠️ БЕЗ измерени числа за тази клетка (%d от нужните %d)"
            % (_n, MIN_N)), True


def _има_числа(seg):
    """Има ли ИЗМЕРЕНИ числа зад тази клетка, или е празна/тънка."""
