...
def main():
        ...
        try:
            ...
            for _вид, _цн, _кга, _през, _гап in _ев2:
                ...
                if _вид == "sl" and not _вц2 and abs(_цн - _сн2["entry"]) > 0.05:
                    guard[_сн2["direction"]] = guard.get(_сн2["direction"], 0) + 1
                _об2 = dict(_сн2)
                _об2["hit"] = dict(_ch2)
    ...
    if should_sig and new_dir and guard.get(new_dir, 0) >= ПАЗАЧ_СТОПОВЕ and trade is None:
        should_sig = False
        _спрян = ("стоп-пазач · %d %s днес в тази посока"
                  % (guard.get(new_dir, 0),
                     "стоп" if guard.get(new_dir, 0) == 1 else "стопа"),
                  "третият опит в същия ден и същата посока исторически влошава деня — "
                  "пазачът се вдига утре")
        notes.append("карта спряна: стоп-пазач (%d от %d стопа днес)"
                     % (guard.get(new_dir, 0), ПАЗАЧ_СТОПОВЕ))
    ...
    if _гейт_обрат:
        ...
        if (not should_sig and bool(actionable) and new_dir
                ...
                and guard.get(new_dir, 0) < ПАЗАЧ_СТОПОВЕ):
            should_sig = True
